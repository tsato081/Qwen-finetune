...
### 推奨: 中間指標も併記する
最終的には「CSVの8フィールド一致」が評価指標ですが、改善点の切り分けを速くするために中間指標も出します。

- 人物候補の再現率（記事内人物の取りこぼし）
- 住所候補の再現率（居住地/本社/犯行場所の取りこぼし）
- `is_offender` のF1（加害者判定の精度）
- `act` の一致率（正規化/表記ゆれを含む）

### 推奨: LLM運用時の最低限ガード（崩れ対策）
- `temperature=0`
- JSONは可能なら `schema` / `grammar` で拘束
- `reason` は本文引用に限定（「本文のこの部分」だけを書き、推測や要約を書かない）
- 出力で `null` と `""` を混ぜない（内部表現は `null` に寄せ、評価直前に `""` へ統一）
